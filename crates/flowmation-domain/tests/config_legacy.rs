...
use flowmation_domain::config::{
    ModelConfig, ModelsConfig, PartialModelsConfig, ProviderConfig, merge_agent_instructions,
};

fn configured_models() -> ModelsConfig {
    ModelsConfig {
...
// Legacy: ConfigService.test.ts — loads first-run scaffold without requiring a model.
#[test]
...
// Legacy: ConfigService.test.ts — merges project aliases and validates their targets.
#[test]
fn merges_project_model_aliases_and_validates_targets() -> Result<(), Box<dyn std::error::Error>> {
    let global = configured_models();
...
fn rejects_ambiguous_unqualified_models() {
    let mut config = configured_models();
